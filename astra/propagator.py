...
def _nrlmsise00_density_njit(
    ...
    the Python ``nrlmsise00_density`` in data_pipeline.py .
...
def _acceleration(
    ...
    if use_drag and alt_km < 1500.0:
        if hf_atmosphere:
            # Both Python and Numba paths now call the same
            ...
            rho_instant = 0.0
        if rho_instant > 0.0:
            # Use imported constant, not hardcoded literal.
            omega_earth = np.array([0.0, 0.0, EARTH_OMEGA_RAD_S])
            v_rel = v - np.cross(omega_earth, r)
...
def _powered_derivative(
    ...
    drag_rho: float,
    drag_H_km: float,
    drag_ref_alt_km: float,  # was missing; caused arity mismatch in Python path
    f107_obs: float,  # Space weather params forwarded to _acceleration
    f107_adj: float,
    ap_daily: float,
    hf_atmosphere: bool,
    include_third_body: bool,
    ...
    Signature now includes f107_obs, f107_adj, ap_daily, hf_atmosphere
    in parity with _powered_derivative_njit.
...
"""
    ...
    # J5/J6 added to Numba production kernel.
    ...
    # --- J5 Perturbation --- # Ref: Vallado §8.7.2; EGM96 coefficient (Lemoine et al. 1998).
    ...
    # --- J6 Perturbation --- # Ref: EGM96 coefficient (Lemoine et al. 1998).
    r13 = r11 * r2
    z6 = z4 * z2
    ...
    # --- Third-Body (Sun & Moon) via Chebyshev Splines ---
    if include_third_body:
        # Piecewise 7-day spline evaluation instead of monolithic
        idx = int((t_jd - t_jd0) / 7.0)
        num_pieces = sun_coeffs.shape[0]
        if idx >= num_pieces:
            idx = num_pieces - 1
...
def _propagator_jacobian_njit(
    ...
    * **F-02**: ``eps_v`` is set to 1e-6 km/s (1 mm/s) matching standard
    ...
    """
    # eps_v = 1e-6 km/s (1 mm/s) — not 1e-4 (100 m/s)
...
def _powered_derivative_njit(
    ...
    if v_mag < 1e-6:
        ...
        # rot_matrix from frames.py defines the ECI -> VNB (or RTN) rotation
    ...
    if len(y) == 43:
        Phi = y[7:].reshape((6, 6))
        # Use _propagator_jacobian_njit (drag-aware) instead of gravity-only Jacobian.
        F = _propagator_jacobian_njit(
...
def _compute_planetary_splines(
    ...
    # Avoid Chebyshev degree cliffs by chunking long propagations
...
def propagate_cowell(
    ...
    if duration_s > 72 * 3600:
        ...
        )  # Previous text incorrectly blamed SGP4 — this is the Cowell integrator.
    ...
    for seg_start_s, seg_end_s, active_burn in segments:
        ...
        if use_drag and use_empirical_drag:
            # Recompute drag_ref_alt_km from the actual current position.
...
def _build_segments(
    ...
    cursor_s = 0.0
    end_s = duration_s
    # Robust overlap detection (SE-G) — build a set of burn
...
def propagate_cowell_batch(
    ...
    pattern that users had to replicate (and which already existed
    *inline* inside :func:`find_conjunctions`).
    ...
    Note:
        ...
        these are NOT separate kwargs. Returns:
