...
def test_atmo_density_monotone_with_altitude():
    ...
    densities = [
        atmospheric_density_empirical(alt, 150.0, 150.0, 15.0)
...
def test_g400_computation_matches_propagator():
    ...
    assert abs(expected_g400 - 8.6842) < 0.001, (
...
def test_geodetic_equator():
    ...
    assert abs(alt[0] - 621.863) < 1.0, f"Equatorial alt should be ~621.9 km, got {alt[0]:.3f} km"
...
def test_geodetic_north_pole():
    ...
    assert abs(alt[0] - orbit_alt) < 5.0, (
...
def test_geodetic_south_pole():
    ...
    assert abs(lat[0] + 90.0) < 0.5, f"South pole lat should be ~-90°, got {lat[0]:.4f}°"
...
def test_teme_to_ecef_conserves_magnitude():
    ...
    r_teme = np.array([[6778.0, 0.0, 0.0],
                       [4000.0, 5000.0, 2000.0]])
...
END Ephemeris
"""
    with patch("astra.conjunction.fetch_synthetic_covariance_stk", return_value=bad_stk):
...
def test_stk_covariance_km_accepted():
    ...
    # km² units — valid
    vals = " ".join(["1.0" if i in (0, 2, 5, 9, 14, 20) else "0.0"
                     for i in range(21)])
    good_stk = f"""
stk.v.11.0
BEGIN Ephemeris
...
END Ephemeris
"""
    with patch("astra.conjunction.fetch_synthetic_covariance_stk", return_value=good_stk):
        with patch("astra.conjunction.SPACEBOOK_ENABLED", True):
            result = load_spacebook_covariance(25544)
