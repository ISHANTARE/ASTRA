...
def test_batch_consistency_se_a():
    ...
    np.testing.assert_allclose(
        state_single.position_km, pos_batch, rtol=1e-12, atol=1e-12,
        err_msg="Position mismatch: propagate_many and propagate_orbit diverged!"
    )
    np.testing.assert_allclose(
        state_single.velocity_km_s, vel_batch, rtol=1e-12, atol=1e-12,
        err_msg="Velocity mismatch: propagate_many and propagate_orbit diverged!"
    )
    
    print("Consistency check PASSED. Batch and single-point propagation are synchronized.")
