import pytest
from astra.debris import filter_altitude, filter_region, filter_time_window, catalog_statistics, make_debris_object
from astra.errors import FilterError, AstraError
...
def test_filter_altitude_returns_subset(catalog_objs):
    ...
    assert len(filtered) == 2  # The 3rd object in small_catalog has very high period/low mean motion? Wait, actually depends on the fake TLE I made.
...
def test_filter_region_polar_orbit_included(catalog_objs):
    ...
    filtered = filter_region(catalog_objs, lat_min_deg=80.0, lat_max_deg=90.0, lon_min_deg=0.0, lon_max_deg=0.0)
